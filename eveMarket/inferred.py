...
from .history import fetch_history, rolling_mean_volume
...
def _rprint(msg: str, *, end: bool = False) -> None:
    rprint("eveMarket.inferred", msg, end=end)
...
def diff_snapshots(
    ...
    *,
    ...
    _rprint(f"[infer] indexed prev snapshot: {len(prev):,} orders", end=True)
    ...
    def _mean_for(region_id: int, type_id: int) -> float:
        ...
        if v is not None:
            return v
        try:
            rows = fetch_history(data_dir, region_id, type_id,
                                 client=client, refresh_after_s=history_refresh_s)
    ...
    with Path(curr_jsonl).open("r", encoding="utf-8") as f:
        for line in f:
            ...
            if orders_seen % 5000 == 0:
                _rprint(f"[pass1] partial-fill scan. orders: {orders_seen:,}. trades: {trades_found:,}")
    ...
    _rprint(f"[pass1] partial-fill scan. orders: {orders_seen:,}. trades: {trades_found:,}", end=True)
    ...
    for i, (oid, prev_row) in enumerate(removed, 1):
        if i % 500 == 0 or i == total_removed:
            ...
            if i > 0:
                ...
                eta_str = "--"
            _rprint(f"[pass2] removed-order check. {i}/{total_removed}. ETA {eta_str}. trades: {trades_found:,}")
        try:
            vp = float(prev_row.get("volume_remain", 0))
        except (TypeError, ValueError):
            continue
    ...
    _rprint(f"[pass2] removed-order check. {total_removed}/{total_removed}. ETA 0m 00s. trades: {trades_found:,}", end=True)
    _rprint(f"[infer] complete in {time.monotonic() - t0:.1f}s. {trades_found:,} trades total", end=True)
